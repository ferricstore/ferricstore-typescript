...
export function registerStoreFlowIntegrationTests(): void {
  ...
  it("covers Flow state-machine repair and index commands", async () => {
    ...
    try {
      ...
      if (completed == null) {
        throw new Error("expected completed rewind flow");
      }
      ...
      if (!isFlowRecord(rewound)) {
        throw new Error("expected rewound Flow record");
      }
      ...
      const rewoundEvent = historyAfter.find((event) => {
        ...
        const eventName = field(fields, "event");
        return eventName != null && text(eventName) === "rewound";
      });
      if (rewoundEvent == null) {
        throw new Error("expected rewind history event");
      }
      ...
      if (!isFlowRecord(restored)) {
        throw new Error("expected restored Flow record");
      }
...
function isFlowRecord(value: unknown): value is { raw?: unknown; state: string; version: number; fencingToken: number | bigint } {
